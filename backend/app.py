...
# ─────────────────────────────────────────────────────────────────────────────
# PAYMENTS BLUEPRINT WIRING
#
# payments.py owns ArchPay/M-Pesa config, STK push, verify, callback, and
# /api/quotations/buy. It's registered here — rather than up near DATABASE/
# EMAIL where the other blueprint (assistant_bp) is registered — because
# init_payments() needs several names as real objects, not just declared:
# issue_dmvic_certificate (DMVIC CONFIGURATION section), monarch_policy_class/
# next_monarch_policy_no (MONARCH POLICY NUMBER SEQUENCES section), and
# safe_error_response (just above). All of those only exist below this point
# in the file, so this is the earliest place the call can safely go.
# ─────────────────────────────────────────────────────────────────────────────
...
init_payments(
    ...
    enqueue=enqueue,
    issue_dmvic_certificate=issue_dmvic_certificate,
    cache_delete_prefix=cache_delete_prefix,
    safe_error_response=safe_error_response,
